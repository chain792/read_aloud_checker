...
RSpec.describe 'Sentences', type: :request do
  ...
  describe 'POST /create' do
    ...
    context 'ログイン前' do
      it 'アクセス制限される' do
        expect{ post api_v1_user_sentences_path, params: params, xhr: true }.to change{ Sentence.count }.by(0)
        expect(response).not_to be_successful
        expect(response).to have_http_status :unauthorized
      end
    end
  ...
  describe 'DELETE /destroy' do
    ...
    context 'ログイン前' do
      it 'アクセス制限される' do
        expect{ delete api_v1_user_sentence_path(sentence_by_me), xhr: true }.to change{ Sentence.count }.by(0)
        expect(response).not_to be_successful
        expect(response).to have_http_status :unauthorized
      end
    end
